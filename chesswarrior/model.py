...
class ChessModel(object):
    ...
    def add_rsnet(self, input_data):
        ...
        block1 = Conv2D(filters=model_config.cnn_filter_num, kernel_size=model_config.cnn_filter_size,
                        padding="same", data_format="channels_first",use_bias=False, kernel_initializer="RandomUniform",
    ...
    def build(self):
        ...
        block1 = Conv2D(filters=model_config.cnn_filter_num, kernel_size=model_config.cnn_first_filter_num,
                        kernel_initializer="RandomUniform", padding="same", data_format="channels_first", activation="relu")(input_data)
        block1 = BatchNormalization(axis=1)(block1)

        for _ in range(model_config.res_layer_num):
            block1 = self.add_rsnet(block1)

        block2_policy = Conv2D(filters=2, kernel_size=1, data_format="channels_first",kernel_initializer="RandomUniform",
        ...
        block2_value = Conv2D(filters=4, kernel_size=1, data_format="channels_first", kernel_initializer="RandomUniform",
                               use_bias=False, kernel_regularizer=l2(model_config.l2_regularizer),
                               activation="relu")(block1)
        ...
        value_out = Dense(units=1, kernel_regularizer=l2(model_config.l2_regularizer), kernel_initializer="RandomUniform",
